...
AI 端和悬浮窗靠三个 txt 文件传话：

- `message.txt` — 玩家消息。悬浮窗**追加**写入，一行一条，格式 `[HH:MM:SS] 内容`。
  玩家可能连发多条，**必须处理文件里的所有行**，全部读完后把文件清空（写入空字符串），
  避免下次重复处理。语音输入转出来的文字也走这个文件。
- `danmaku.txt` — AI 回复/弹幕。**追加**写入（不要覆盖！），一行一条纯文本，
  悬浮窗每 1.5 秒读取显示后自动清空。追加模式下连发多条、内容重复都不会丢。
- `description.txt` — 最新画面描述，截图脚本覆盖写入，只读即可。
