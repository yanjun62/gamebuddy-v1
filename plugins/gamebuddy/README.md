...
Game Buddy's original code and project documentation are licensed under `AGPL-3.0-only`. Modified versions offered to users over a network must provide those users access to the corresponding source as required by section 13 of the GNU Affero General Public License v3. Third-party game names, trademarks, and source materials remain under their respective owners' terms; see `THIRD_PARTY_NOTICES.md`.
